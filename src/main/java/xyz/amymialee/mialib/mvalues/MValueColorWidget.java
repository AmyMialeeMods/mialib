...
public class MValueColorWidget extends MValueWidget<Integer> {
	...
	@Override
	public void onClick(@NotNull Click click, boolean doubled) {
		var client = MinecraftClient.getInstance();
		if (click.x() < this.getX() + 18) {
			this.value.send(this.value.type.defaultValue);
		} else {
			client.setScreen(new ColorPickerOverlayScreen(client.currentScreen, this, this.value.type.defaultValue));
		}
	}
	...
	protected void renderWidget(@NotNull DrawContext context, int mouseX, int mouseY, float delta) {
		...
		var color = HexFormat.fromHexDigits(this.value.getValueAsString());
		var brightness = (ColorHelper.getRedFloat(color) + ColorHelper.getGreenFloat(color) + ColorHelper.getBlueFloat(color)) / 3f;
		int valueX = -client.textRenderer.getWidth(valueText), valueY = -client.textRenderer.fontHeight / 2;
		if (1f - brightness > 0.4) { //outline
			var outlineColor = 1f - brightness > 0.5f ? 0xFF000000 : 0xFFFFFFFF;
