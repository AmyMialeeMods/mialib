...
public class MValueEnum<T extends Enum<T>> extends MValueType<T> {
    ...
    protected void registerServerCommand(@NotNull MValue<T> value) {
        CommandRegistrationCallback.EVENT.register((dispatcher, access, environment) -> {
            dispatcher.register(CommandManager.literal("mvalue").requires(CommandManager.requirePermissionLevel(value.permissionCheck)).then(CommandManager.literal(value.id.toString()).executes(ctx -> {
            ...
            for (var enumConstant : value.type.defaultValue.getDeclaringClass().getEnumConstants()) {
                dispatcher.register(CommandManager.literal("mvalue").requires(CommandManager.requirePermissionLevel(value.permissionCheck)).then(CommandManager.literal(value.id.toString())
    ...
    protected void registerClientCommand(@NotNull MValue<T> value) {
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, access) -> {
            dispatcher.register(ClientCommandManager.literal("mvalue").requires(CommandManager.requirePermissionLevel(value.permissionCheck)).then(ClientCommandManager.literal(value.id.toString()).executes(ctx -> {
                ctx.getSource().sendFeedback(Text.translatable("commands.mvalue.query", value.getText(), value.getValueAsString()));
                return 1;
            })));
            for (var enumConstant : value.type.defaultValue.getDeclaringClass().getEnumConstants()) {
                dispatcher.register(ClientCommandManager.literal("mvalue").requires(CommandManager.requirePermissionLevel(value.permissionCheck)).then(ClientCommandManager.literal(value.id.toString())
