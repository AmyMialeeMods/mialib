...
public class MValueColor extends MValueType<Integer>
{
	...
	@Override
	public boolean set(MValue<Integer> mValue, Integer value)
	{
		mValue.value = value;
		return true;
	}
	...
	@Override
	public NbtCompound writeNbt(NbtCompound compound, MValue<Integer> value)
	{
	...
	@Override
	public void readNbt(NbtCompound compound, MValue<Integer> value)
	{
		value.value = compound.getInt("value", this.defaultValue);
	}
	
	@Override
	public JsonElement writeJson(MValue<Integer> value)
	{
		return new JsonPrimitive(Integer.toHexString(value.get()));
	}
	
	@Override
	public void readJson(JsonElement json, MValue<Integer> value)
	{
		try
		{
	...
	protected void registerServerCommand(MValue<Integer> value)
	{
		CommandRegistrationCallback.EVENT.register(
				(dispatcher, access, environment) ->
						dispatcher.register(CommandManager.literal("mvalue").requires(source -> source.hasPermissionLevel(value.permissionLevel))
													.then(CommandManager.literal(value.id.toString())
																  .then(CommandManager.argument("value", HexColorArgumentType.hexColor()))
																  .executes(ctx -> {
																	  value.set(HexColorArgumentType.getArgbColor(ctx, "value"));
	...
	protected void registerClientCommand(MValue<Integer> value)
	{
		ClientCommandRegistrationCallback.EVENT.register(
				(dispatcher, access) ->
						dispatcher.register(ClientCommandManager.literal("mvalue").requires(source -> source.getPlayer().hasPermissionLevel(value.permissionLevel))
