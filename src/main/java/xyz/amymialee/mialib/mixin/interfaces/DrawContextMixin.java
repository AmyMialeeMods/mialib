...
public abstract class DrawContextMixin implements MDrawContext {
    ...
    @Override
    public void mialib$drawTexture(@NotNull Function<Identifier, RenderLayer> renderLayers, Identifier sprite, float x1, float x2, float y1, float y2, float z, float u1, float u2, float v1, float v2, int color) {
        ...
        vertexConsumer.vertex(matrix4f, x1, y1, z).texture(u1, v1).color(color);
        vertexConsumer.vertex(matrix4f, x1, y2, z).texture(u1, v2).color(color);
        vertexConsumer.vertex(matrix4f, x2, y2, z).texture(u2, v2).color(color);
        vertexConsumer.vertex(matrix4f, x2, y1, z).texture(u2, v1).color(color);
    }
